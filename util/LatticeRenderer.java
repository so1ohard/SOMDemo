...
public class LatticeRenderer extends JPanel {
	...
	// grid as an image
	public void render(SOMLattice lattice, int iteration, Vector inputs, String topCluster) {
...
//		float cellHeight = 2;
		...
		g2.drawString("Iteration: " + String.valueOf(iteration) + "  Nearest Cluster: " + topCluster, 5, 15);
		for (int i = 0; i < inputs.size(); i++) {
			SOMVector curInput = (SOMVector) inputs.elementAt(i);
			SOMNode bmu = lattice.getBMU(curInput);
			g2.drawString(String.valueOf(i),bmu.getX()*cellWidth,bmu.getY()*cellHeight);
		}
		g2.dispose();
		repaint();
	}
